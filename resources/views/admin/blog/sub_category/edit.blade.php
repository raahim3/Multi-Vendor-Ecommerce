      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <form action="{{ route('admin.blog.sub_categories.update', $sub_category->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-control" name="category_id" required>
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" @if($category->id == $sub_category->blog_category->id) selected @endif>{{ $category->title }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <span class="validated_txt">{{ $message }}</span>
